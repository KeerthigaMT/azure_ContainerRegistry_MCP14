...
func Registries_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRegistries_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName",
		mcp.WithDescription("Deletes a container registry."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Registries_deleteHandler(cfg),
	}
}
