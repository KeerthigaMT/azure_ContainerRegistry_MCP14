...
func Registries_listcredentialsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ContainerRegistry/registries/%s/listCredentials%s", cfg.BaseURL, subscriptionId, resourceGroupName, registryName, queryString)
...
func CreateRegistries_listcredentialsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_listCredentials",
		mcp.WithDescription("Lists the login credentials for the specified container registry."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Registries_listcredentialsHandler(cfg),
	}
}
