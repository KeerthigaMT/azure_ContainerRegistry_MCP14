...
func Registries_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRegistries_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName",
		mcp.WithDescription("Creates a container registry with the specified parameters."),
		...
		mcp.WithObject("registry", mcp.Required(), mcp.Description("The parameters for creating a container registry.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Registries_createHandler(cfg),
	}
}
