...
func Registries_regeneratecredentialHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ContainerRegistry/registries/%s/regenerateCredential%s", cfg.BaseURL, subscriptionId, resourceGroupName, registryName, queryString)
...
func CreateRegistries_regeneratecredentialTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_regenerateCredential",
		mcp.WithDescription("Regenerates one of the login credentials for the specified container registry."),
		...
		mcp.WithObject("regenerateCredentialParameters", mcp.Required(), mcp.Description("Specifies name of the password which should be regenerated -- password or password2.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Registries_regeneratecredentialHandler(cfg),
	}
}
