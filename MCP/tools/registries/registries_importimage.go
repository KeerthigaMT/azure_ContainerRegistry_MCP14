...
func Registries_importimageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ContainerRegistry/registries/%s/importImage%s", cfg.BaseURL, subscriptionId, resourceGroupName, registryName, queryString)
...
func CreateRegistries_importimageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_importImage",
		mcp.WithDescription("Copies an image to this container registry from the specified container registry."),
		...
		mcp.WithObject("parameters", mcp.Required(), mcp.Description("The parameters specifying the image to copy and the source container registry.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Registries_importimageHandler(cfg),
	}
}
