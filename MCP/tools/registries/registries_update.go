...
func Registries_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ContainerRegistry/registries/%s%s", cfg.BaseURL, subscriptionId, resourceGroupName, registryName, queryString)
...
func CreateRegistries_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName",
		mcp.WithDescription("Updates a container registry with the specified parameters."),
		...
		mcp.WithObject("registryUpdateParameters", mcp.Required(), mcp.Description("The parameters for updating a container registry.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Registries_updateHandler(cfg),
	}
}
