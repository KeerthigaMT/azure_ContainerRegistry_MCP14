...
func Replications_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplications_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_replications_replicationName",
		mcp.WithDescription("Deletes a replication from a container registry."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replications_deleteHandler(cfg),
	}
}
