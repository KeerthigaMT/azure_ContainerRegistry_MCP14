...
func Replications_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplications_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_replications_replicationName",
		mcp.WithDescription("Creates a replication for a container registry with the specified parameters."),
		...
		mcp.WithObject("replication", mcp.Required(), mcp.Description("The parameters for creating a replication.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Replications_createHandler(cfg),
	}
}
