...
func Webhooks_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		webhookNameVal, ok := args["webhookName"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: webhookName"), nil
		}
		webhookName, ok := webhookNameVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: webhookName"), nil
		}
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ContainerRegistry/registries/%s/webhooks/%s%s", cfg.BaseURL, subscriptionId, resourceGroupName, registryName, webhookName, queryString)
...
func CreateWebhooks_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_webhooks_webhookName",
		mcp.WithDescription("Creates a webhook for a container registry with the specified parameters."),
		...
		mcp.WithString("webhookName", mcp.Required(), mcp.Description("The name of the webhook.")),
		mcp.WithObject("webhookCreateParameters", mcp.Required(), mcp.Description("The parameters for creating a webhook.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Webhooks_createHandler(cfg),
	}
}
