...
func Webhooks_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateWebhooks_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_webhooks_webhookName",
		mcp.WithDescription("Updates a webhook with the specified parameters."),
		...
		mcp.WithObject("webhookUpdateParameters", mcp.Required(), mcp.Description("The parameters for updating a webhook.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Webhooks_updateHandler(cfg),
	}
}
