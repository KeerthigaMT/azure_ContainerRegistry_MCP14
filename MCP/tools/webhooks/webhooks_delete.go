...
func Webhooks_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateWebhooks_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_webhooks_webhookName",
		mcp.WithDescription("Deletes a webhook from a container registry."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Webhooks_deleteHandler(cfg),
	}
}
