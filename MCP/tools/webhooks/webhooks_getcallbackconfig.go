...
func Webhooks_getcallbackconfigHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ContainerRegistry/registries/%s/webhooks/%s/getCallbackConfig%s", cfg.BaseURL, subscriptionId, resourceGroupName, registryName, webhookName, queryString)
...
func CreateWebhooks_getcallbackconfigTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_webhooks_webhookName_getCallbackConfig",
		mcp.WithDescription("Gets the configuration of service URI and custom headers for the webhook."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Webhooks_getcallbackconfigHandler(cfg),
	}
}
