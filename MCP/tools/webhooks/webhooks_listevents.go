...
func Webhooks_listeventsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ContainerRegistry/registries/%s/webhooks/%s/listEvents%s", cfg.BaseURL, subscriptionId, resourceGroupName, registryName, webhookName, queryString)
...
func CreateWebhooks_listeventsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_webhooks_webhookName_listEvents",
		mcp.WithDescription("Lists recent events for the specified webhook."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Webhooks_listeventsHandler(cfg),
	}
}
