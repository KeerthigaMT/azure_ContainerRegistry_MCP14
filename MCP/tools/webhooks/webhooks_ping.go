...
func Webhooks_pingHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ContainerRegistry/registries/%s/webhooks/%s/ping%s", cfg.BaseURL, subscriptionId, resourceGroupName, registryName, webhookName, queryString)
...
func CreateWebhooks_pingTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerRegistry_registries_registryName_webhooks_webhookName_ping",
		mcp.WithDescription("Triggers a ping event to be sent to the webhook."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Webhooks_pingHandler(cfg),
	}
}
